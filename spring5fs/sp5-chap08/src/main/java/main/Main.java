...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		while (true) {
			...
			if (cmd.startsWith("new ")) {
				processNewCmd(cmd.split(" "));
			} else if (cmd.startsWith("change ")) {
				processChangeCmd(cmd.split(" "));
			} else if (cmd.equals("list")) {
				processListCmd();
			} else if (cmd.startsWith("info ")) {
				processInfoCmd(cmd.split(" "));
			} else {
				printHelp();
			}
		}
	...
	private static void processNewCmd(String[] arg) {
		if (arg.length != 5) {
			printHelp();
			return;
		}
	...
	private static void processChangeCmd(String[] arg) {
		if (arg.length!=4) {
			printHelp();
			return;
		}
	...
	private static void processInfoCmd(String[] arg) {
		if (arg.length != 2) {
			printHelp();
			return;
		}
